...
function collection (name, schema) {
  ...
  const Cstr = function (data, refChecks = true) {
    if (refChecks) {
      refs.forEach((ref) => {
  ...
  Cstr.initPromise = new Promise((resolve, reject) => {
    ...
    stream.on('data', (record) => {
      ...
      if (Object.keys(constructors).length === constructorsWithClosedStreams.length) {
        debug('all constructors closed streams')
        restoringMode = true
        resolvingDocs.forEach(getAllRefsSync)
        restoringMode = false
        _.values(constructors).forEach((c) => {
          if (c._resolve) { // if this constructor is not yet resolved,
            c._resolve()  // resolve it
            debug(`enresolved cstr ${c.name} resolved synchronously`)
          }
        })
      } else {
